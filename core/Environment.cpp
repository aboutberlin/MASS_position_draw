...
Initialize()
{
	...
	std::ofstream fout("joint_states_log.csv", std::ios::out);  // 显式指定写模式
	if (fout.is_open()) {
		fout << "L_Hip_pos,L_Hip_vel,R_Hip_pos,R_Hip_vel,L_Knee_pos,L_Knee_vel,R_Knee_pos,R_Knee_vel,L_Knee_tau,R_Knee_tau\n";
		fout.close();
	} else {
		std::cerr << "[Logger] ❌ Failed to create joint_states_log.csv" << std::endl;
	}
...
{	
	...
	LogJointStatesToFile("joint_states_log.csv");
...
void Environment::LogJointStatesToFile(const std::string& filename)
{
    auto skel = mCharacter->GetSkeleton();
    ...
    Eigen::VectorXd tau = mDesiredTorque; 

	fout << p[idx_L_Hip] << "," << v[idx_L_Hip] << ","
	<< p[idx_R_Hip] << "," << v[idx_R_Hip] << ","
	<< p[idx_L_Knee] << "," << v[idx_L_Knee] << ","
	<< p[idx_R_Knee] << "," << v[idx_R_Knee] << ","
	<< mDesiredTorque[idx_L_Knee] << "," << mDesiredTorque[idx_R_Knee] << std::endl;


    fout.close();

    // 🔔 每次写入时输出一行调试信息（可选限制输出频率）
    static int line_counter = 0;
    line_counter++;
    if (line_counter % 100 == 0) {
        std::cout << "[Logger] ✅ Written line " << line_counter << " to " << filename << std::endl;
    }
}
